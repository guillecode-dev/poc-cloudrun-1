...
import { OrdersService, Order, CreateOrderDto } from '../../../services/orders.service';
...
@Component({
  ...
  template: `
    <div class="page-container">
      ...
      <div class="page-card" style="padding: 1.25rem; margin-bottom: 1.5rem;">
  ...
  styles: [`
    ...
    .section-title { font-size: 1.15rem; font-weight: 600; color: #0f3460; margin: 0 0 1rem 0; }
    .page-card { background: #fff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,.08); }
    .order-form { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 0.75rem 1.25rem; align-items: start; }
    .form-actions { display: flex; gap: 0.75rem; align-items: center; padding-top: 0.25rem; grid-column: 1 / -1; }
    .table-wrapper { overflow-x: auto; }
    .order-badge { background: #e8f0fe; color: #1a3a6b; padding: 2px 6px; border-radius: 4px; font-size: .8rem; font-family: monospace; }
    .status-chip { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: .78rem; font-weight: 600; text-transform: uppercase; }
...
export class OrdersComponent implements OnInit, AfterViewInit {
  ...
  loadOrders(): void {
    ...
    this.ordersService.getOrders().subscribe({
      next: orders => { this.dataSource.data = orders; this.loading = false; },
  ...
  submitOrder(): void {
    ...
    this.ordersService.createOrder(dto).subscribe({
      next: newOrder => {
        this.dataSource.data = [newOrder as Order, ...this.dataSource.data];
